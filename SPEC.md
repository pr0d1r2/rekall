...
V30: `[signals]` WEIGHTS decide CLASS, â¥ SHARPNESS. Class is a BALANCE -- directive against hedge -- â´ it takes a weight & a DEADBAND, & that deadband IS V10's "I do not know". Sharpness is a LADDER of KINDS (Â§I: `M2` = runner needs ONE human-set parameter) â´ â¥ a score: a sum cannot say WHICH KIND of runner a statement admits, & rounding one to a rung INVENTS the property V10 assigns to the STATEMENT. `show` prints â signal WITH its weight â´ the balance is ARGUABLE.
