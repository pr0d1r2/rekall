...
V38: a fired `M` rule ADVISES, ⊥ BLOCKS, & a runner that ⊥ finish in BOUNDED time ⊥ fired. TWO reasons, each sufficient: this runs in the TOOL-CALL path ∴ a hung runner stalls the harness on every call · a WRONG rule that blocks costs the user their WORK, while a wrong rule that advises costs a LINE. Nothing is lost -- the runner still GATES at commit (V2's wiring) ∴ the hook is the EARLY word & the gate is the LAST. REJECTED: `permissionDecision: deny` (one buggy extracted rule then wedges every tool call, & unwedging it means editing the corpus mid-task).
V37: `hook` fires an `M` rule ONLY where it carries a TRIGGER, & runs ONLY what the LEDGER names. An EMPTY `M` block = GATE-ONLY -- ⊥ broken, ⊥ nagged at by `check`: the runner's home is the gate (V2) & a trigger is how it ADDITIONALLY arrives uninvited (§G) ∴ both hold, neither is a special case. LEDGER-named is the teeth: a script DROPPED into `.rekall/rules/` is an ORPHAN `check` reports, ⊥ something a tool call runs. Trust is a git hook's -- user-authored, user's repo -- ∴ the surface is SELECTION, ⊥ execution. REJECTED: firing ∀ `M` on ∀ call (a spawn per rule per call; V11 would count invocations ⊥ fires); `hook` never firing `M` (⊥ §I; §G's "arrives at its trigger point" would hold for skills only).
V34: `hook` writes the FIRE COUNTER & NOTHING ELSE -- ⊥ a span, ⊥ an artifact, ⊥ a row's content. V7 named three writers & left V11's counter with NO author ∴ `--dead` reports every artifact dead forever & R13's argument collapses. V20 ⊥ applies: it guards the DESTRUCTIVE path, a counter deletes nothing, & `hook` is unattended ∴ a prompt there hangs the harness V20 protects. `recall` counts NOTHING: the number ! mean LOADED, ⊥ asked about. It ! survive CONCURRENT hooks -- one per tool call ∴ read-modify-write RACES. REJECTED: a second store (⊥ V11; two stores disagree -- V8); `hook` report-only (nothing counts).
...
T39|.|`hook` FIRES an `M` rule that carries a trigger; empty block = gate-only; ADVISES ⊥ blocks, bounded time|V37,V38,V2,T13
