...
V37: `hook` fires an `M` rule ONLY where that rule carries a TRIGGER, & runs ONLY what the LEDGER names. An `M` block left EMPTY means GATE-ONLY -- ⊥ broken, ⊥ nagged about by `check`: the runner's home is the gate (V2's wiring) & a trigger is how a rule ADDITIONALLY arrives uninvited (§G). ∴ both are true at once & neither is a special case. Executing only LEDGER-named artifacts is the teeth: a script DROPPED into `.rekall/rules/` is an ORPHAN `check` reports (V29's neighbour), ⊥ something a tool call runs. The trust is a git hook's -- user-authored, in the user's own repo -- ∴ the surface is the SELECTION, ⊥ the execution. REJECTED: firing ∀ `M` on ∀ call (a spawn per rule per tool call, & V11 would count invocations ⊥ fires); `hook` never firing `M` (⊥ §I, & §G's "arrives at its trigger point" would hold for skills only).
...
T39|.|`hook` FIRES an `M` rule that carries a trigger; empty block = gate-only|V37,V2,T13
