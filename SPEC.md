...
R16|Codex transcript shape|MEASURED 2026-09-05 over 20 rollouts on a Linux box running `codex` unattended: a turn is `{type:response_item, payload:{type:message, role, content:[{type,text}]}}`. Roles COUNTED: `user` 40 · `developer` 20, exactly one per file & 4.4k chars, the injected `AGENTS.md` · `assistant` 95. The only public writeup calls `developer` the human ∴ it is WRONG, & building on it would have mined the rules file instead of the corrections (`catch:V51`). Field paths & role names only; ⊥ a byte of message content was read (V15)|MEASURED, internal
