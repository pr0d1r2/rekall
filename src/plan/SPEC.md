...
V71: an extraction that leaves a source file with ⊥ a statement SAYS SO, at `plan` & again at `apply` BEFORE the prompt. `.:V1` wants the pointer ∴ the husk is LEGAL, & in a one-concept-per-file corpus it is also a skill that still exists, still loads & says NOTHING -- a shape nobody pictures from a `net` line. The note is per-FILE, computed from the WHOLE corpus ∵ the question is what would be LEFT, which the steps cannot answer alone; & RECOMPUTED at `apply` ∵ a saved plan (`.:V19`) may predate a statement the file has since gained.
...
B29|2026-09-06|extracting `set/skills/gnu/sed.md` reduced the file to a heading & a pointer, & ⊥ `plan` ⊥ `apply` said anything: the corpus is ONE CONCEPT PER FILE ∴ the whole file was the statement. MEASURED while dogfooding a real extraction into `set-and-setting`. `net +65` is true & tells a reader nothing about the husk it leaves|V71
