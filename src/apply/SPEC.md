...
V60: an index ENTRY is a FINDING AID & extraction leaves it ALONE. MEASURED 2026-09-05 against a real memory dir: the hook is a SUMMARY (`-- no feature branches in this repo`) & the file states the rule at length; the format's own rule is ONE LINE per memory, NEVER the content. ∴ the entry is a table-of-contents line that existed BEFORE the extraction & would exist after -- V1 is ⊥ engaged, ∵ extraction did ⊥ create a second statement, & the line the reader needs to FIND the file is ⊥ a duplicate of it. REJECTED, & this rule once said the opposite: maintaining the entry means REWRITING a human's summary, which is AUTHORING (`.:V5`), for a duplication that was measured ⊥ to exist. What the index DID need is `statement:V62` -- ⊥ offering the entry as an extraction at all, which is a REFUSAL & needs ⊥ model.
...
T76|x|REJECTED on measurement: the index hook is a SUMMARY, ⊥ a copy of the extracted rule, so there is ⊥ duplication to maintain away & rewriting it would be authoring. The destructive half became `statement:T78`|V60,`statement:V62`
