...
V59: `apply` RESOLVES the source it NAMED, wherever it lives. V16 makes a memory dir read-only UNLESS `apply` named the file ∴ a `~`-prefixed or absolute source is EXTRACTABLE, ⊥ forbidden. MEASURED 2026-09-05: `edit_sources` does `base.join(&src)` ∴ `~/...` becomes `<project>/~/...` & the write dies `No such file or directory` at exit 2, naming ⊥ file. `scan`·`show`·`plan` all handle the same row ∴ the HEADLINE capability -- mine your agent memory, extract from it -- is broken at the last step only, & the crash was mistaken for a guard (`B20`). CORRECTED: an earlier draft of this rule said REFUSE, which contradicts the V16 it cites. What is refused is what cannot be RESOLVED, & it says which file & why.
V60: an INDEXED corpus is edited WHOLE or ⊥ at all. A memory dir carries `MEMORY.md`, one line per file, & extraction that deletes a span leaves that line pointing at prose ⊥ there -- or leaves it accurate & pays the context cost TWICE, which is the defect this crate removes. ∴ where an index NAMES the source, `apply` maintains BOTH or REFUSES the row & says the index is unhandled. rekall does ⊥ own this convention (it is the harness's) ∴ it is DETECTED, ⊥ assumed: ⊥ index found = ordinary prose, & the rule sleeps.
V61: memory LEAVES only by a HUMAN's word, twice. FIRST whether: a `feedback_*` note about how someone wants to be TALKED TO scores identically to one about hooks, ∴ class is a CANDIDATE & ⊥ a permission, & moving a private note into a TRACKED `.rekall/` changes who can read it. SECOND when: the entry STANDS until the artifact has FIRED (`log:V55`'s counter, ⊥ a green gate -- `log:B11` says that measures another path), then retirement is its own act. `src/issue:V54` one level EARLIER: delete-on-extract reopens the gap the extraction closed, & here the file is someone's private memory. REJECTED: extracting on class alone, which is a tool deciding what a person meant.
