...
V47: the AGENT is NAMED, ⊥ SNIFFED. `hook` & `catch` take `--agent <name>`; a payload is ⊥ probed for a discriminator. Codex sends `turn_id` & `permission_mode` where Claude Code sends neither ∴ a sniffer rests on ABSENCE, & absence is exactly what the next harness release changes -- & a detector that guesses wrong is SILENT, which is `.:V26`'s failure moved into the adapter. The caller ALREADY writes the hook config line ∴ one word costs nothing. An UNKNOWN name is exit 2, ⊥ a fallback: defaulting to `claude` on a Codex box IS the silent no-op this rule exists to refuse. REJECTED: sniffing · a configured DEFAULT agent (a guess with a config file around it).
